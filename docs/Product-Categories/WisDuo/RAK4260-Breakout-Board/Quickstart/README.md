...
Before going through each step in the installation guide of the RAK4260 Breakout Module, make sure to prepare the necessary items listed below:
...
### Interfacing with RAK4260 Breakout Module
...
<rk-img
  src="/assets/images/wisduo/rak4260-breakout-board/quickstart/interfacing/jlink-to-rak4260-breakout.png"
  width="75%"
  caption="J-Link to RAK4260 Breakout Module"
/>

<rk-img
  src="/assets/images/wisduo/rak4260-breakout-board/quickstart/interfacing/jlink-connection.png"
  width="75%"
...
After filling in the necessary information, press the "**Add application**". If the page is the same as figure 11 then, you have successfully registered your application.
...
1. **Device ID** - this is the unique identifier for your RAK4260 Breakout Module in your application. You need to enter this manually.
...
These can be obtained from the Device Overview page, where they are grouped one after the other for convenience (Figure 15).
...
As these are randomly generated by TTN we need to update our RAK4260 Breakout Module with their values in order to be able to register it with the network. However, as mentioned before, you can only do this directly in the source code, compile it and then
...
- The next step is to select the right “**Target Device**”, to match the chip at the core of the RAK4260 Breakout Module. Press the button with the dots on it in Figure 24 and you will be redirected to the selection screen.
...
- Select “**Atmel**” as the manufacturer via the drop-down menu and navigate to the **ATSAMR34J18** list entry. This is the chip at the core of the RAK4260 Breakout Module, so we need to instruct the J-Flash tool that we are going to be working with it as shown in Figure 25.
...
- After you have selected the band, the device will automatically try to connect to the network using the **Device EUI, Application EUI and Application Key** set in the firmware configuration file. These are also reported in the window in Figure 34.
